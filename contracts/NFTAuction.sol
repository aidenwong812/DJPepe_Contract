...
contract NFTAuction is Ownable, ERC721Holder {
    ...
    ) public payable onlyTokenOwner(_collectionId, _tokenId) ReentrancyGuard {
    ...
    function bidOnAuction(
        ...
        if (bidsLength > 0)
            swapAndTransfer(myTokenAddr, lastFrom, lastBidPrice, false);
    ...
    function finalizeAuction(
        ...
        if (bidsLength == 0) {
            ...
            if (_managers[myOwner] == true) {
                uint256 _memberLevel = getMemberLevel(lastFrom);
                _memberFee =
                    (lastBidPrice * memberDiscount[_memberLevel]) /
                    PERCENTS_DIVIDER;
            }
            ...
            uint256 _sellerValue = lastBidPrice -
                _adminFee -
                _royalty -
                _memberFee;

            swapAndTransfer(myTokenAddr, feeAddress, _adminFee, true);
            swapAndTransfer(myTokenAddr, lastFrom, _memberFee, false);
            swapAndTransfer(
                myTokenAddr,
            ...
            if (_sellerValue > 0) {
                ...
                for (uint256 i = 0; i < splitsLen; i++) {
                    uint256 _splitSend = (_sellerValue * splits[i].commission) /
                        PERCENTS_DIVIDER;
